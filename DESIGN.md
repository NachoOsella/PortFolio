...
The fixed navigation is `72px` tall on desktop and `64px` on mobile. Home and full-height route surfaces account for the navigation height. The desktop home hero fills the remaining viewport and centers its title block. The project gallery occupies one viewport and uses a full-width horizontal track. Practice and writing use asymmetric two-column grids with large fluid gaps. Reading pages use a `210px` table of contents column and a `760px` reading column within a `1120px` layout.
